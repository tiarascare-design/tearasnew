        ...
        navEls.forEach(el => {
            el.addEventListener('click', (e) => {
                try {
                    ...
                    // Immediate visual feedback: update active class synchronously
                    try { navEls.forEach(n => n.classList.remove('active')); } catch(_) {}
                    ...
                    // Fallback: ensure a render occurs shortly after navigation in case Router handling is async
                    try { setTimeout(() => { try { renderApp(); } catch(_){} }, 80); } catch(_) {}
                } catch(_) {}
            });
        });
    } catch(_) {}
...
document.body.addEventListener('submit', async e => {
    ...
    const tmpTotal = (pricesIncludeGst
        ...
        const purchaseData = {
            ...
            items,
            // Store subtotal as ex-GST for consistency; prefer displayed values when available
            subtotal: (function(){ try { const txt = (document.getElementById('purchaseSubtotal')?.textContent||'').replace(/[^0-9.\-]/g,''); return txt ? parseFloat(txt) : gstComputed.subtotalEx; } catch(_) { return gstComputed.subtotalEx; } })(),
            gstBreakdown: (function(){ try { const txt = (document.getElementById('purchaseGst')?.textContent||'').replace(/[^0-9.\-]/g,''); const gstVal = txt ? parseFloat(txt) : (gstBreakdownToStore?.total || 0); const gb = Object.assign({}, gstBreakdownToStore); gb.total = +(gstVal.toFixed ? gstVal.toFixed(2) : gstVal); return gb; } catch(_) { return gstBreakdownToStore; } })(),
...
function closeNewProductModal() {
    try {
        // If the modal was opened from a product <select>, reset that select back to the placeholder
    ...
    try {
        ...
        lingering.forEach(s => {
            try { if (s && s.value === 'new') { s.value = ''; try { s.dispatchEvent(new Event('change', { bubbles: true })); } catch(_){} } } catch(_) {}
        });
    } catch(_) {}
    const modal = document.getElementById('newProductModal');
   if (!modal) return;
